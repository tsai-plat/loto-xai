...
export class XaiSseFetch {
  ...
  constructor(apiPath: string, options: XaiStreamableOptions) {
    ...
    if (!globalFetch) throw new Error('your enviroment no fetch.');
  ...
  >(data: D, cb?: StreamablePrepareCallback): Promise<XaiSseFetch> {
    ...
    try {
      ...
      else this.dispatchEvent('error', SseFetchError.createFromError(e));
    } finally {
      this.fetchInprogress = false;
    }
  ...
  removeListener(eventName: SseEventType, handler: SseCallbackFn): XaiSseFetch {
    ...
    if (idx >= 0) this.eventListeners[eventName].splice(idx, 1);
  ...
  private _registListeners(
    options: XaiStreamableOptions & { eventDataParsed?: boolean },
  ) {
    const {
      checkAuth,
      eventDataParsed,
  ...
  private pushMessageCache(message: SseMessageType) {
    if (!this.sseCaches.messages) this.sseCaches.messages = [];
  ...
  >(data: D) {
    ...
    if (q.length) this.reqCached.question = q;
  }
    ...
    return function onLine(line: Uint8Array, fieldLength: number) {
      if (line.length === 0) {
        ...
        if (message.data.length) {
          ...
          if (chunkData?.content) that.appendResult(chunkData.content);
